    ...
    <div className="flex flex-col gap-4 w-64 shrink-0 border-r border-neutral-200 bg-neutral-50 p-4 h-full overflow-y-auto">
      {readOnly && (
        <div className="bg-amber-50 border border-amber-200 text-amber-800 text-xs rounded-md px-3 py-2">
          Your account has view-only access. You can look around and export a PDF, but can&apos;t change this layout.
        </div>
      )}
      ...
      <div className="border-t border-neutral-200 pt-4">
        ...
        <p className="text-xs text-neutral-500 mt-1">
          {readOnly ? "Downloads a scaled floor plan of the last saved version." : "Saves, then downloads a scaled floor plan."}
        </p>
      </div>
      ...
      <div className="border-t border-neutral-200 pt-4 text-xs text-neutral-500 leading-relaxed">
        {readOnly
          ? "Click an object to see it highlighted. Scroll to pan, or use the zoom slider above."
          : "Drag to move. Click an object then drag the rotate handle to turn it. Scroll to pan, or use the zoom slider above."}
      </div>
    </div>
  );
}
